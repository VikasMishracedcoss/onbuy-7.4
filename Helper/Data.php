...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getRequest($url, $authentication = false)
    {
       ...
       // $url="https://api.onbuy.com/v2/listings/check-winning?site_id=2000&skus=$ch";
        $request = null;
        $header = [];
        $response = null;
        try {
...
///////////////////////////////////////
    ...
    public function getOrders()
    {
        ...
        try {
            ...
            $startDate = strtotime(date('Y-m-d').' -30 days');
    ...
    public function updateAccountVariable($accountId = null)
    {
        ...
        $this->apiUrl = (($account) ? trim($account->getAccountEnv()) : '') == 'sandbox' ? self::API_URL_SANDBOX
            :self::API_URL;
        $this->token = ($account) ? trim($account->getAccessToken()) : '';
        $this->tokenExpiration = ($account) ? trim($account->getTokenExpiration()) : '';
        $this->secret = ($account) ? trim($account->getConsumerSecret()) : '';
        $this->key = ($account) ? trim($account->getConsumerKey()) : '';
